...
use std::arch::x86_64::{
    __m256, _mm256_add_ps, _mm256_blend_ps, _mm256_castps_pd, _mm256_castps_si256, _mm256_fmadd_ps,
    _mm256_hadd_ps, _mm256_i32gather_ps, _mm256_loadu_ps, _mm256_mul_ps, _mm256_permute_ps,
    _mm256_permutevar8x32_ps, _mm256_set1_ps, _mm256_set_ps, _mm256_storeu_ps,
};

use crate::{simd_f64::f64x4, simd_i32::i32x8};
...
impl f32x8 {
    ...
    pub fn splat(v: f32) -> Self {
        unsafe { _mm256_set1_ps(v) }.into()
    }

    #[inline]
    pub fn permute<const IMM: i32>(self) -> Self {
        unsafe { _mm256_permute_ps::<IMM>(self.v) }.into()
    }

    #[inline]
    pub fn permute4x64<const IMM: i32>(self) -> Self {
        self.to_raw_f64().permute4x64::<IMM>().to_raw_f32()
    }

    #[inline]
    pub fn permute_var(self, idx: i32x8) -> Self {
        unsafe { _mm256_permutevar8x32_ps(self.v, idx.into()) }.into()
    }
    ...
    #[inline]
    pub unsafe fn from_ptr(a: *const f32) -> Self {
        _mm256_loadu_ps(a).into()
    }
    ...
    pub fn to_raw_i32(self) -> i32x8 {
        unsafe { _mm256_castps_si256(self.v) }.into()
    }

    #[inline]
    pub fn to_raw_f64(self) -> f64x4 {
        unsafe { _mm256_castps_pd(self.v) }.into()
    }
    ...
    #[inline]
    pub unsafe fn store_ptr(self, a: *mut f32) {
        _mm256_storeu_ps(a, self.v)
    }

    #[inline]
    pub fn hadd(self, other: Self) -> Self {
        unsafe { _mm256_hadd_ps(self.v, other.v) }.into()
    }

    #[inline]
    pub fn mul_add(self, mul: Self, add: Self) -> Self {
        unsafe { _mm256_fmadd_ps(self.v, mul.v, add.v) }.into()
    }
}

impl From<__m256> for f32x8 {
    #[inline]
    fn from(v: __m256) -> Self {
        Self { v }
    }
}

impl Into<__m256> for f32x8 {
    fn into(self) -> __m256 {
        self.v
    }
}
...
impl Mul<f32x8> for f32x8 {
    ...
    fn mul(self, rhs: f32x8) -> Self::Output {
        unsafe { _mm256_mul_ps(self.v, rhs.v) }.into()
    }
}
...
impl Add<f32x8> for f32x8 {
    ...
    fn add(self, rhs: f32x8) -> Self::Output {
        unsafe { _mm256_add_ps(self.v, rhs.v) }.into()
    }
}
